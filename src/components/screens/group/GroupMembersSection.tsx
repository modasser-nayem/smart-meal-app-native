...
const ROLE_CONFIG: Record<MemberRole, { bg: string; text: string }> = {
   Owner: { bg: "bg-primary/10", text: "text-primary" },
   Manager: { bg: "bg-info/10", text: "text-info" },
   Member: { bg: "bg-surface", text: "text-secondary-300" },
};
            ...
            {members.map((member, index) => {
               ...
               return (
                  <TouchableOpacity
                     ...
                     activeOpacity={0.75}
                     className={`flex-row items-center gap-3 px-4 py-4 active:bg-surface ${member.role === "Owner" ? "border-l-[3px] border-primary" : ""} ${index < members.length - 1 ? "border-b border-outline" : ""}`}
                  >
                     <View className="relative">
                        ...
                        <View
                           className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-surface-container ${member.isActive ? "bg-accent" : "bg-outline"}`}
                        />
                     </View>
               ...
               <>
                  ...
                  >
                     <Typography className="text-secondary-300 text-sm">
                        {t("members.moreMembers", { count: remaining })}
                     </Typography>
