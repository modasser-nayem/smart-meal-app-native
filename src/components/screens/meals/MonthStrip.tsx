...
export const MonthStrip = ({
   ...
   const months = Array.from({ length: 12 }).map((_, i) =>
      setMonth(new Date(selectedMonth), i),
   );
      ...
      <View className="mb-6 px-6 flex-row items-center">
         ...
         >
            <MaterialCommunityIcons
               ...
               color="#F59E0B"
            />
            <Typography className="text-xs font-black text-on-surface">
               {currentYear}
            </Typography>
         </TouchableOpacity>
         ...
         >
            {months.map((monthDate, index) => {
               ...
               const isCurrentMonth = index === currentMonthIdx && getYear(new Date()) === currentYear && getMonth(selectedMonth) !== index;
               ...
               return (
                  <TouchableOpacity
                     ...
                     className={cn(
                        "px-5 h-11 rounded-2xl items-center justify-center mr-2 border transition-all",
                        isSelected
                  ...
                  >
                     <Typography
                        className={cn(
                           ...
                           isSelected ? "text-surface" : "text-on-surface/60",
                        )}
                     >
